...
class RdStationCrmClient
{
    ...
    protected function extractErrorMessage(\Illuminate\Http\Client\Response $response): string
    {
        ...
        return $body['message'] ?? $body['error'] ?? 'Erro na comunicação com RD Station CRM (HTTP ' . $response->status() . ')';
    }
    ...
    {
        return $this->makeRequest('PATCH', "organizations/{$id}", ['data' => $data]);
    }

    public function upsertOrganizacao(string $nome): array
    {
        ...
        if ($result['status'] === 'success') {
            ...
            if (is_array($items) && !empty($items)) {
                ...
                if (isset($org['id'])) {
                    Log::info('RD Station CRM: Organização encontrada', ['id' => $org['id'], 'nome' => $nome]);
                    return ['status' => 'success', 'data' => $org, 'action' => 'found'];
                }
            }
        }

        Log::info('RD Station CRM: Criando nova organização', ['nome' => $nome]);
        $createResult = $this->criarOrganizacao(['name' => $nome]);
    ...
    {
        return $this->makeRequest('PATCH', "contacts/{$id}", ['data' => $data]);
    }

    public function upsertContato(string $nome, ?string $telefone, ?string $email, ?string $organizationId): array
    {
        $filter = 'name:"' . addslashes($nome) . '"';
    ...
    {
        return $this->makeRequest('PATCH', "deals/{$id}", ['data' => $data]);
    }
    ...
    {
        return $this->makeRequest('POST', "deals/{$dealId}/deal_products", ['data' => $data]);
    }
