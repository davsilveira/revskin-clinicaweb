...
- Conferir no resumo: `stubs legado criados` deve ser **0** ou só os sem equivalente (Apostila, SUICO, Tonalite `___` sem fototipo, Mezzotono).
- **TONALITE-___-G30**: resolvido automaticamente quando `paciente.fototipo` existe no dump legado (~5% dos itens); demais permanecem stub.
