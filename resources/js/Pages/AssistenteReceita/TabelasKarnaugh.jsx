        ...
        <DashboardLayout title="Tabelas Karnaugh">
            <div className="py-4 lg:py-6 px-0">
                <PageHeader
                    ...
                    description="Gerencie as tabelas de mapeamento de produtos por caso clínico"
                    actions={
                        <button
                            type="button"
                ...
                {tabelas.length === 0 ? (
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
                        ...
                        <button
                            type="button"
                            onClick={() => setShowDrawer(true)}
                            className="min-h-[44px] px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700"
                        >
                        ...
                        <ResponsiveEntityList
                                            ...
                                            <tr key={tabela.id} className={!tabela.ativo ? 'bg-gray-50 opacity-60' : ''}>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {tabela.padrao && (
                                                            <span className="px-2 py-0.5 text-xs font-medium bg-emerald-100 text-emerald-800 rounded-full">Padrão</span>
                                                        )}
                                                    </div>
                                                    {tabela.descricao && <p className="text-sm text-gray-500 mt-1">{tabela.descricao}</p>}
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${tabela.ativo ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                                                    ...
                                                    <div className="flex items-center justify-end gap-2">
                                                        ...
                                                        {deletingId === tabela.id && (
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <button type="button" onClick={() => handleExcluir(tabela)} className="px-2 py-1 bg-red-600 text-white text-xs rounded hover:bg-red-700">Sim</button>
                                                                <button type="button" onClick={() => setDeletingId(null)} className="px-2 py-1 bg-gray-200 text-gray-700 text-xs rounded hover:bg-gray-300">Não</button>
                                                            </div>
                                                        )}
                                                    </div>
                                                </td>
                            ...
                            mobile={
                                <div className="divide-y divide-gray-200">
                                    {tabelas.map((tabela) => (
                                        <div key={tabela.id} className={`p-4 max-w-full ${!tabela.ativo ? 'bg-gray-50/80' : ''}`}>
                                            <div className={`rounded-lg border border-gray-200 bg-white overflow-hidden ${!tabela.ativo ? 'opacity-90' : ''}`}>
                                                <button
                                                    type="button"
                                                ...
                                                >
                                                    <div className="flex flex-wrap items-start gap-2">
                                                        <span className="font-medium text-gray-900 break-words">{tabela.nome}</span>
                                                        {tabela.padrao && (
                                                            <span className="px-2 py-0.5 text-xs font-medium bg-emerald-100 text-emerald-800 rounded-full shrink-0">Padrão</span>
                                                        )}
                                                        <span className={`px-2 py-1 text-xs font-medium rounded-full shrink-0 ${tabela.ativo ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                                                            {tabela.ativo ? 'Ativa' : 'Inativa'}
                                                        </span>
                                                    </div>
                                                    {tabela.descricao && <p className="text-sm text-gray-600 mt-2">{tabela.descricao}</p>}
                                                    <p className="text-sm text-gray-600 mt-2">
                                                        <span className="font-medium text-gray-900">{tabela.casos_count || 0}</span> casos
                                                    </p>
                                                    {tabela.arquivo_original && (
                                                        <p className="text-xs text-gray-500 mt-1 break-all" title={tabela.arquivo_original}>
                                                            {tabela.arquivo_original}
                                                        </p>
                                                    )}
                                                </button>
                                                {deletingId === tabela.id ? (
                                                    <div className="flex flex-wrap items-center justify-end gap-2 px-2 py-3 border-t border-gray-200 bg-gray-50/80">
                                                        <span className="text-sm text-gray-700 mr-auto">Confirmar exclusão?</span>
                                                        <button type="button" onClick={() => handleExcluir(tabela)} className="min-h-[44px] px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700">
                                                            Sim
                                                        </button>
                                                        <button type="button" onClick={() => setDeletingId(null)} className="min-h-[44px] px-4 py-2 bg-gray-200 text-gray-800 text-sm font-medium rounded-lg hover:bg-gray-300">
                                                            Não
                                                        </button>
                                                    </div>
                                                ) : (
                                                    <div className="flex flex-wrap items-center justify-end gap-1 px-2 py-2 border-t border-gray-100 bg-gray-50/60">
                                                        ...
                                                        {!tabela.padrao && tabela.ativo && (
                                                            <button
                                                                type="button"
                                                                onClick={() => handleDefinirPadrao(tabela)}
                                                                className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center p-2 text-gray-600 hover:text-amber-600 hover:bg-amber-50 rounded-lg"
                                                        ...
                                                        <button
                                                            ...
                                                            className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center p-2 text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg"
                                                            aria-label={tabela.ativo ? 'Desativar' : 'Ativar'}
                                                        >
                                                            {tabela.ativo ? (
                                                                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                ...
                                                                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.752 11.168l-3.197-1.798A1 1 0 0010 10.382v3.236a1 1 0 001.555.832l3.197-1.798a1 1 0 000-1.664z" />
                                                        ...
                                                        {!tabela.padrao && (
                                                            <button
                                                                type="button"
                                                                onClick={() => setDeletingId(tabela.id)}
                                                                className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center p-2 text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-lg"
                                                                aria-label="Excluir"
                                                            >
                                                                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
